...
Ver 16: Se elimina la fase deepsleep para solo poner un delay. Para facilitar la operacion en la feria de educacion técnica
...
#define TIME_VEL_PICKUP 5000 // tiempo para interrupciones anemómetro en ms
  ...
  */
  ...
  if((err!=0) && (cardType != CARD_NONE))
  {
    ...
    //Serial.println(buff0);
     db_write("Vel_Viento",vel_viento,2);
   ...
   // Serial.print(direction);
    ...
    InfoMeteo += ",";
    if(vel_viento >0)
      db_write_char("Dir_Viento",direction);
    else
      db_write_char("Dir_Viento","---");
